...
# sending poc attached email
def send_mail(address_op):

	#Addind porlem and (ploblem discription) if given to content of email
	content = mail_sub_st
	ad = ""
	#Address details
	...
	#Attaching Address details to content of email
	content +=ad

	#Reading Image by selected path storing it in dictionary (image_path as key and image raw data as key value)
	#in img_data
	img_data = {}
	#ts is for attaching time stamp to email content with filename
	ts = ""
	i=0
	...
	# Adding time stamp to content of email
	content+="\n\nTime Stamp:"+ts
...
#checking if PoC or Address are given or not
